...
class micThread (threading.Thread):
	...
	def __init__(self, threadID, threadName, audio):
	...
	def run(self):
		...
		waveFile.writeframesraw(b''.join(frames))
		waveFile.close()
...
class musicThread (threading.Thread):
	...
	def run(self):
		...
		stream2 = audio2.open(format=FORMAT, channels=CHANNELS, rate=RATE, input=True, 
							frames_per_buffer=CHUNK, 
...
music_thread = musicThread(1, "musicThread", audio)
mic_thread = micThread(2, "micThread", audio)
